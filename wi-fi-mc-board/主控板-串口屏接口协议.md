...
- 设备地址和版本信息
    ```json
    { "json_type": "info", "mc_board_type" : "wifi", "mac_address": "94:A4:08:89:5A:54", "version": "v1  v1.aaa.bbb" }
    ```
    其中，  
    “mc_board_type”表示主控板类型，取值：  
    - “wifi”：BW16 Wi-Fi板；  
    - “eth”：CH32以太网板。  

    “version”中的aaa表示电源驱动板版本号，bbb表示BW16/CH32软件版本号。  
...
- modbus寄存器值
    ...
    ```
    发送的寄存器：4~12, 14, 15, 21, 102 ~ 105, 108~110  
    消息发送时机：  
    1. 主控板收到屏幕发来的读寄存器请求后，将寄存器值发给屏幕。  
    2. 主控板收到对任何寄存器的写操作之后，如果写成功，立刻发送此消息给屏幕。  
    3. 主控板定时（10秒）发送。如果由于上述1或2的原因刚发送过一次，则从发送之后重新开始定时器计时。  
...
- 写寄存器  
    ...
    消息发送时机：屏幕根据自己内部实现逻辑可选发送此命令。  
    注意：扩展寄存器（编号在100以上的寄存器）中，只允许对110号（测距信息补偿）进行写入。  
...
3. 调试/测试用消息  
    ```json
    { "json_type": "eng_and_dbg", "allow_force_expo_ig_dist": 0, "dbg_lvl": 0}
    ```
    各字段的取值：  
    - allow_force_expo_ig_dist  
    0：测距结果大于特定值（20cm）时才允许曝光。默认值。  
    1：取消上述限制。  

    - dbg_lvl  
    -1：关闭所有log消息打印。  
    0 ~ 3：DEBUG， INFO， WARN， ERROR。  

    消息中可以只包含需要设置的字段。例如，如果只想修改debug消息级别，则只需要包含dbg_lvl字段即可。
