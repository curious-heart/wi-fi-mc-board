...
WiFiClient::WiFiClient(tPortMode portMode):
    _sock(MAX_SOCK_NUM)
{
    _is_connected = false;
    recvTimeout = 3000;
...
WiFiClient::WiFiClient(uint8_t sock, tPortMode portMode)
{
    _sock = sock;
    if ((sock >= 0) && (sock != 0xFF)) {
        _is_connected = true;
...
WiFiClient::WiFiClient(uint8_t sock, tPortMode portMode, tBlockingMode blockMode)
{
    _sock = sock;
    if ((sock >= 0) && (sock != 0xFF)) {
        _is_connected = true;
...
int WiFiClient::available()
{
    ...
    while (true) {
        if (clientdrv.availData(_sock) > 0) {
            ...
            if (err == EAGAIN) {
                if (NON_BLOCKING_MODE == _is_blocked) {
                continue;
                } else {
